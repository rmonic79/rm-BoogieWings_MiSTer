//============================================================================
//  crt_adjust_sys.sv  —  "CRT Adjust" (SYS-SIDE variant)
//
...
(
    ...
    //
    // 2026-09-18 (BoogieWings, stessa cura di Seibu Cup): il registro unico vale
    // solo se il vb in ingresso e' una riga in ritardo sul DE, come su Night
    // Slashers. Su BW vb e DE cambiano allo STESSO campione (misurato sul flusso
    // vero di mister_sim): nella prima riga di blanking il read rigioca ancora
    // l'ultima riga attiva e vb_active la spegneva -> riga sorgente 240 mai
    // emessa, a ogni V-Size compreso lo 0, e al suo posto una riga nera in cima.
    // Ora la finestra si ricava dal CONTENUTO: la riga che il read rigioca e'
    // attiva se, quando e' stata scritta, ha avuto pixel attivi (fronte di lhb).
    // Vero per costruzione con qualunque fase di vb_in.
    reg line_act_wr;   // la riga in scrittura ha avuto pixel attivi
    reg line_act_rd;   // ... quella che il read sta rigiocando ora
    initial begin line_act_wr = 0; line_act_rd = 0; end
    always @(posedge clk) if (pxl_cen) begin
        if (hs_rise_in) begin
            line_act_rd <= line_act_wr;
            line_act_wr <= 1'b0;
        end else if (lhb & ~lhb_l) line_act_wr <= 1'b1;
    end
    wire vb_active = ~line_act_rd;
    ...
    // ------------------------------------------------------------------
    //  Il positivo e' un ritardo di N righe. Il negativo non puo' anticipare:
    //  ritarda di (righe del quadro - N), cioe' un giro meno N. Vale solo se il
    //  quadro in ingresso ha davvero quelle righe. Con crt_vsize a monte in PVM
    //  il quadro ne ha VTOTAL+vsize: con la costante VTOTAL il primo scatto
    //  negativo spostava il VSync di 1+vsize righe invece di 1 (il salto a -1
    //  col PVM acceso; in Cabinet le righe totali non cambiano e non saltava).
    //  2026-09-18 (stessa soluzione di Seibu Cup): le righe del quadro si
    //  CONTANO fra due fronti di vs_in, e lo shift register copre il quadro piu'
    //  lungo che il V-Size possa produrre.
    localparam integer VSHREG = VTOTAL + 32;

    wire vs_rise_in = pxl_cen && vs_in && !vs_in_q;   // vs_in_q e' registrato su pxl_cen
    reg        vs_seen;
    reg  [9:0] vl_cnt, vl_meas;
    initial begin vs_seen = 0; vl_cnt = 0; vl_meas = 10'(VTOTAL); end
    always @(posedge clk) begin
        if (hs_rise_in) begin
            if (vs_seen || vs_rise_in) begin          // prima riga del quadro nuovo
                vl_meas <= vl_cnt + 10'd1;            // righe del quadro appena chiuso
                vl_cnt  <= 10'd0;
                vs_seen <= 1'b0;
            end else if (vl_cnt != 10'd1023) vl_cnt <= vl_cnt + 10'd1;
        end else if (vs_rise_in) vs_seen <= 1'b1;
    end
    // finche' la misura non e' plausibile vale il parametro
    wire [9:0] vl_use = (vl_meas == 10'd0 || vl_meas > 10'(VSHREG)) ? 10'(VTOTAL) : vl_meas;

    wire [9:0] vshift_tap = voffset[5]
        ? (vl_use + {{4{voffset[5]}}, voffset})       // negativo -> righe del quadro - |N|
        : {4'd0, voffset};                            // positivo -> N
    reg [VSHREG-1:0] vsync_line_shreg;
    initial vsync_line_shreg = 0;
    always @(posedge clk) if (hs_rise_in)
        vsync_line_shreg <= {vsync_line_shreg[VSHREG-2:0], vs_in};
    reg vs_shifted;
    initial vs_shifted = 0;
    always @(posedge clk) if (hs_rise_in)
        vs_shifted <= (vshift_tap == 10'd0) ? vs_in : vsync_line_shreg[vshift_tap - 10'd1];
    ...
    always @(posedge clk) begin
        if (bypass) begin
            ...
            if (pxl2_cen) begin
                ...
                // in ramo attivo si pubblica il flusso EMESSO (come hb_out con
                // ~pass_q): col vb in ingresso l'ultima riga emessa veniva
                // riannullata a valle.
                vb_out <= vb_active;
            end
        end
