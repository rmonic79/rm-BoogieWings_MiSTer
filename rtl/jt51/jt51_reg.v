...
module jt51_reg(
    ...
    output  reg     op31_no,
    output  reg     op31_acc,
    // Savestate (auto_ss, modello F2). 1589 bit = csr_op(1344) + reg_ch(208) + kon(32) +
    // cur(5). Layout: [1343:0]csr_op [1551:1344]reg_ch [1583:1552]kon [1588:1584]cur.
    // (kon/koff/csm_state/csm_cnt sono dichiarati ma MAI assegnati = dead code, non salvati.
    //  zero/half/op31_* derivano da cur -> rigenerati.)
    input      [1588:0] auto_ss_in,
    output     [1588:0] auto_ss_out,
    input               auto_ss_wr
);
...
always @(posedge clk, posedge rst) begin : up_counter
    ...
    else if(auto_ss_wr) cur <= auto_ss_in[1588:1584];   // restore
    else if(cen) begin
        cur     <= next;
        zero    <= next== 5'd0;
        half    <= next[3:0] == 4'd0;
...
assign auto_ss_out[1588:1584] = cur;
...
jt51_kon u_kon (
    ...
    .keyon_II  (keyon_II  ),
    .auto_ss_in ( auto_ss_in[1583:1552]  ),
    .auto_ss_out( auto_ss_out[1583:1552] ),
...
jt51_csr_op u_csr_op(
    ...
    .rate2          (  rate2_II     ),
    .rrate          (  rrate_II     ),
    .auto_ss_in     (  auto_ss_in[1343:0]  ),
    .auto_ss_out    (  auto_ss_out[1343:0] ),
    .auto_ss_wr     (  auto_ss_wr          )
);
...
jt51_reg_ch u_csr_ch(
    ...
    .ams_VII    ( ams_VII       ),
    .pms        ( pms_I         ),
    .auto_ss_in ( auto_ss_in[1551:1344]  ),
    .auto_ss_out( auto_ss_out[1551:1344] ),
    .auto_ss_wr ( auto_ss_wr             )
);
