...
module jt51_noise(
    ...
    output            out,
    output reg [11:0] mix,
    // Savestate (auto_ss, modello F2). 24 bit: [15:0]lfsr [16]last_lfsr0 [21:17]cnt
    //  [22]update [23]nfrq_met. mix (output) si ricalcola -> NON salvato.
    input      [23:0] auto_ss_in,
    output     [23:0] auto_ss_out,
    input             auto_ss_wr
);
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        cnt  <= 5'b0;
    end else if( auto_ss_wr ) begin   // restore
        cnt      <= auto_ss_in[21:17];
        update   <= auto_ss_in[22];
        nfrq_met <= auto_ss_in[23];
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        lfsr       <= auto_ss_in[15:0];
        last_lfsr0 <= auto_ss_in[16];
    end else if(cen) begin
        lfsr       <= { fb, lfsr[15:1] };
...
assign auto_ss_out = { nfrq_met, update, cnt, last_lfsr0, lfsr };
