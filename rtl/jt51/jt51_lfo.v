...
module jt51_lfo(
    ...
    output  reg [7:0]   am,
    output  reg [7:0]   pm,
    // Savestate (auto_ss, modello F2). Salvo la FASE del LFO = 43 bit:
    //  [3:0]cnt1 [7:4]cnt3 [11:8]bitcnt [26:12]cnt2 [42:27]val.
    //  I derivati (out1/out2/am/pm/sign) si ricalcolano dalla fase + parametri (nel register
    //  file gia' salvato) in pochi cen -> NON salvati (regola "riprende in poco").
    input      [42:0]   auto_ss_in,
    output     [42:0]   auto_ss_out,
    input               auto_ss_wr
);
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        cnt3_step <= 0;
        bitcnt    <= 4'h8;
    end else if( auto_ss_wr ) begin   // restore fase
        cnt1   <= auto_ss_in[3:0];
        cnt3   <= auto_ss_in[7:4];
        bitcnt <= auto_ss_in[11:8];
        cnt2   <= auto_ss_in[26:12];
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        out2      <= 16'd0;
        integ_c   <= 0;
    end else if( auto_ss_wr ) begin   // restore fase val
        val <= auto_ss_in[42:27];
    end else if( cen ) begin
...
assign auto_ss_out = { val, cnt2, bitcnt, cnt3, cnt1 };
