...
entity HUC6280_AG is
    port( 
        ...
        AA     		: out std_logic_vector(15 downto 0);
		  ...
		  -- Savestate (auto_ss, pattern F2): SS_WR=0 -> trasparente (comportamento normale);
		  -- SS_WR=1 (a CPU ferma in paused_safe) -> i registri caricano da SS_DI.
		  SS_DO			: out std_logic_vector(32 downto 0);  -- PCr(16)+AAL(8)+AAH(8)+SavedCarry(1)
		  SS_DI			: in  std_logic_vector(32 downto 0);
		  SS_WR			: in  std_logic
    );
end HUC6280_AG;
...
begin
	...
	begin
		...
		if RST_N = '0' then
			...
			if SS_WR = '1' then
				PCr <= SS_DI(15 downto 0);   -- restore (a CPU ferma): carica PC dal savestate
			elsif CE = '1' then
				PCr <= NextPC;
			end if;
		end if;
	...
	begin
		if RST_N = '0' then
			...
			if SS_WR = '1' then
				AAL        <= SS_DI(23 downto 16);   -- restore: carica AAL/AAH/SavedCarry dal savestate
				AAH        <= SS_DI(31 downto 24);
				SavedCarry <= SS_DI(32);
			elsif CE = '1' then
	...
	-- Savestate output: ordine bit deve combaciare con SS_DI (PCr|AAL|AAH|SavedCarry).
	SS_DO <= SavedCarry & AAH & AAL & PCr;
