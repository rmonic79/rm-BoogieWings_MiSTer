...
module jt6295_serial(
    ...
    // Savestate (auto_ss, modello F2). Stato PERSISTENTE: u_cnt (CSR posizioni lettura ADPCM
    // per canale, 4*42=168 bit) + ch (4 bit, canale corrente) = 172. busy/ack/pipe_* si
    // rigenerano dal CSR in pochi cen (non persistenti).
    input      [171:0]  auto_ss_in,
    output     [171:0]  auto_ss_out,
    input               auto_ss_wr
);
...
always @(posedge clk, posedge rst) begin
    if(rst)
        ...
        if(auto_ss_wr) ch <= auto_ss_in[171:168];   // restore (ultimo = priorita')
    end
end
...
assign auto_ss_out[171:168] = ch;   // ch coperto qui; [167:0] dall'sh_rst u_cnt
...
jt6295_sh_rst #(.WIDTH(CSRW), .STAGES(4) ) u_cnt(
    ...
    .din    ( csr_in    ),
    .drop   ( csr_out   ),
    .auto_ss_in ( auto_ss_in[167:0]  ),   // CSR: posizioni lettura ADPCM per canale (persistente)
    .auto_ss_out( auto_ss_out[167:0] ),
