...
// Per 32 slot persistenti con Main stock: 8 sotto-slot per regione.
// Layout regione (2MB):
//   +0x00            : header regione Main-facing {size_words[63:32], cnt[31:0]}
//   +0x10 + k*0x3F000: sotto-slot k=0..7 (immagine memory_stream autonoma,
//                      header proprio + stato ~165KB << 252KB)
//   8 * 0x3F000 = 0x1F8000, piu' l'header: dentro i 2MB con 32KB di avanzo.
// Save slot N: memory_stream scrive il sotto-slot; poi il mini-FSM qui sotto
// aggiorna l'header di regione (cnt+1, size 0x7E002 word = payload da +8 a fine
// sotto-slot 7) -> e' QUESTO che fa scrivere il file al Main: senza header
...
wire [31:0] sub_addr    = region_base + 32'h00000010 + ({29'd0, index[2:0]} * 32'h0003F000);
    ...
    if (reset) begin
        ...
        case (hstate)
            ...
            H_WR: if (!ddr.busy) begin
                ...
                h_wdata <= {32'h0007E002, hdr_cnt + 32'd1};
                hstate  <= H_WR_WAIT;
            end
