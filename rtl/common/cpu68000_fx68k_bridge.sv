...
(
	...
	output wire        iack,
	output wire  [2:0] fc,           // FC2:FC0 (function code)
...
assign fc   = {fx_fc2, fx_fc1, fx_fc0};
...
`ifdef USE_FX68K_CORE
	...
	// Reset post-stretcher: fx68k pwrUp deve essere alto per N enPhi cycles per
	// inizializzare microAddr/microLatch/nanoLatch. In HW Quartus init=0 mitiga
	// il problema, ma in sim i reg partono X. Counter pilotato da clk (NON da
	// ce_cpu, che dipende da dtack_cen che potrebbe essere ancora in reset).
	// `reset` esterno parte alto, posso usare clk per contare giù post-reset.
	reg [7:0] fx_init_cnt = 8'd255;
	wire fx_init_done = (fx_init_cnt == 8'd0);
	always @(posedge clk) begin
		if (reset)              fx_init_cnt <= 8'd255;
		else if (!fx_init_done) fx_init_cnt <= fx_init_cnt - 8'd1;
	end
	wire fx_reset = reset | ~fx_init_done;
