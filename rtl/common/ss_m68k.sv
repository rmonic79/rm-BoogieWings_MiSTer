	...
	case (ss_state)
		...
		SST_RESTORE_WAIT_READ: begin
			if (ss_busy & ss_mem_read) begin
				ss_mem_read <= 1'b0;
			end else if (~ss_busy & ~ss_mem_read & slot_empty) begin
				// Sotto-slot mai scritto: memory_stream non ha applicato NIENTE.
				// Tornare a IDLE significa gioco intatto; proseguire farebbe reset
				// CPU + reset-vector da uno stato stantio = partita rotta da un
				// load a vuoto.
				ss_state <= SST_IDLE;
